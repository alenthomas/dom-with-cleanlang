...
def process_middleware(middleware_tuple, request, response):
    '''
    middleware_tuple = (middleware_function, PRE, POST)
    '''
    if middleware_tuple[1]:
        return middleware_tuple[0](request, response)
    elif middleware_tuple[2]:
        return middleware_tuple[0](request, response)
    else:
...
def add_middleware(func, PRE=True, POST=False):
    """ADD middlewares
    """
    MIDDLEWARES.append((func, PRE, POST))
...
async def request_handler(request):
    ...
    response = {}
    if MIDDLEWARES:
        for middleware_tuple in MIDDLEWARES:
            request, response = process_middleware(middleware_tuple, request, response)
